...
passports.forEach((passport) => {
  ...
  let fieldData = fields.map((field) => {
    return field.split(":");
  });

  for (let i = 0; i < fieldData.length; i++) {
    const [key, value] = fieldData[i];
    ...
    if (key === "hgt") {
      const metric = value.slice(value.length - 2, value.length);
    ...
    if (key === "hcl" && value.charAt(0) === "#") {
      const color = value.slice(1, value.length);
      if (color.length !== 6) continue;
      console.log("color", color.match(/[a-f,0-9]/g));
      if (color && color.match(/[a-f,0-9]/g)?.length === 6) {
        validPass[key] = true;
      }
    }
    ...
    if (key === "pid" && value.match(/[0-9]/g).length === 9) {
      validPass[key] = true;
    }
  }
