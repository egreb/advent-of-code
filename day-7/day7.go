...
func main() {
	input := `vibrant aqua bags contain 1 shiny magenta bag, 2 muted teal bags, 1 dim magenta bag, 1 muted chartreuse bag.
...
dark lavender bags contain 2 striped blue bags, 5 posh blue bags, 2 plaid green bags.`
	...
	for _, rr := range rules {
		bags := strings.Split(rr, "contain")
		container := fmt.Sprintf("%s", strings.Join(strings.Split(bags[0], " "), " "))

		content := strings.Split(bags[1], ",")

		for _, c := range content {
			if strings.Contains(c, "shiny gold") {
				validContainers[strings.Join(strings.Split(container, " ")[:2], " ")] = true
			}
		}
	}
	...
	for _, rr2 := range rules {
		bags := strings.Split(rr2, "contain")
		container := fmt.Sprintf("%s", strings.Join(strings.Split(bags[0], " ")[:2], " "))
		content := strings.Split(bags[1], ",")

		found := false
		for i := range validContainers {
			if strings.Contains(container, i) {
				found = true
			}
			for _, bagString := range content {
				if strings.Contains(bagString, i) {
					fmt.Println("container", container, "bagString", bagString)
					found = true
					break
				}
			}
